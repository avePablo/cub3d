...
typedef struct s_raycast
{
	...
	double		delta_x; // Distancia que el rayo tiene que recorrer para pasar de una línea vertical a la siguiente
	double		delta_y; // Distancia que el rayo tiene que recorrer para pasar de una línea horizontal a la siguiente
...
void			validate_player(char **map); // Valida que haya exactamente un jugador en el mapa
void			validate_map_closed(char **map); // Valida que el mapa esté cerrado (no haya espacios vacíos alrededor de los pasillos)
int				is_config_line(char *line);
int				is_map_line(char *line);
int				is_empty_line(char *line);
int				is_valid_cell(char **map, int y, int x);
void			init_player_from_map(t_game *g); // Inicializa la posición y dirección del jugador a partir del mapa
